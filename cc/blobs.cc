...
void acmacs::chart::Blobs::calculate(const acmacs::LayoutInterface& layout, const Stress<double>& stress)
{
    ...
    for (size_t point_no = 0; point_no < layout_for_blobs.number_of_points(); ++point_no) {
        ...
        layout_for_blobs.set(point_no, layout.get(point_no));
    }
...
void acmacs::chart::Blobs::calculate(const acmacs::LayoutInterface& layout, const PointIndexList& points, const Stress<double>& stress)
{
    if (layout.number_of_dimensions() != 2)
        throw std::runtime_error("wrong number of dimensions for blobs");
    acmacs::Layout layout_for_blobs{layout};
...
void acmacs::chart::Blobs::calculate_for_point(size_t point_no, acmacs::Layout& layout, const Stress<double>& stress, double initial_stress)
{
