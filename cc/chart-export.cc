...
int main(int argc, char* const argv[])
{
    ...
    try {
        ...
        //     json = to_json::v1::object_append(json, "map_distances", to_json::v1::raw(to_json::v1::array(distances.begin(), distances.end(), [](const auto& row) { return
        //     to_json::v1::raw(to_json::v1::array(row.begin(), row.end())); })));
        // }
        // if (opt.column_bases) {
        //     if (auto fcb = chart->projection(opt.projection)->forced_column_bases(); fcb) {
        //         const auto fcb_data = fcb->data();
        //         json = to_json::v1::object_append(json, "column_bases", to_json::v1::raw(to_json::v1::array(fcb_data.begin(), fcb_data.end())));
        //     }
        //     else {
        //         auto cb_data = chart->computed_column_bases(acmacs::chart::MinimumColumnBasis{})->data();
        //         json = to_json::v1::object_append(json, "column_bases", to_json::v1::raw(to_json::v1::array(cb_data.begin(), cb_data.end())));
        //     }
        // }
...
//         Options opt(argc, argv);
//         auto chart = acmacs::chart::import_from_file(opt.chart, acmacs::chart::Verify::None);
//         auto json = to_json::v1::object();
//         if (opt.names) {
//             auto antigens = chart->antigens();
//             json = to_json::v1::object_append(json, "antigens", to_json::v1::raw(to_json::v1::array(antigens->begin(), antigens->end(), [](const auto& antigen) { return antigen->full_name(); })));
//             auto sera = chart->sera();
//             json = to_json::v1::object_append(json, "sera", to_json::v1::raw(to_json::v1::array(sera->begin(), sera->end(), [](const auto& serum) { return serum->full_name(); })));
//         }
//         if (opt.titers) {
//             auto titers = chart->titers();
//             std::vector<std::vector<size_t>> values(titers->number_of_antigens(), std::vector<size_t>(titers->number_of_sera(), 0UL));
//             for (const auto& titer : titers->titers_existing())
//                 values[titer.antigen][titer.serum] = titer.titer.value_with_thresholded();
//             json = to_json::v1::object_append(json, "titers", to_json::v1::raw(to_json::v1::array(values.begin(), values.end(), [](const auto& row) { return to_json::v1::raw(to_json::v1::array(row.begin(), row.end())); })));
//         }
//         if (opt.logged_titers) {
//             auto titers = chart->titers();
//             std::vector<std::vector<double>> values(titers->number_of_antigens(), std::vector<double>(titers->number_of_sera(), -1));
//             for (const auto& titer : titers->titers_existing())
//                 values[titer.antigen][titer.serum] = titer.titer.logged_with_thresholded();
//             json = to_json::v1::object_append(json, "logged_titers", to_json::v1::raw(to_json::v1::array(values.begin(), values.end(), [](const auto& row) { return to_json::v1::raw(to_json::v1::array(row.begin(), row.end())); })));
//         }
//         if (opt.map_distances) {
//             auto layout = chart->projection(opt.projection)->layout();
//             const auto number_of_antigens = chart->number_of_antigens(), number_of_sera = chart->number_of_sera();
...
//                 for (size_t sr_no = 0; sr_no < number_of_sera; ++sr_no)
//                     distances[ag_no][sr_no] = layout->distance(ag_no, sr_no + number_of_antigens, -1); // -1 for no distance
//             json = to_json::v1::object_append(json, "map_distances", to_json::v1::raw(to_json::v1::array(distances.begin(), distances.end(), [](const auto& row) { return to_json::v1::raw(to_json::v1::array(row.begin(), row.end())); })));
//         }
//         if (opt.column_bases) {
//             if (auto fcb = chart->projection(opt.projection)->forced_column_bases(); fcb) {
//                 const auto fcb_data = fcb->data();
