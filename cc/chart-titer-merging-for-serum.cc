...
int main(int argc, char* const argv[])
{
    ...
    try {
        ...
        std::cout << "SR " << opt.serum_no << ' ' << chart->serum(opt.serum_no)->name_full() << ' ' << static_cast<std::string_view>(chart->serum(opt.serum_no)->passage()) << '\n';
        ...
        for (size_t antigen_no = 0; antigen_no < chart->number_of_antigens(); ++antigen_no) {
            if (std::any_of(acmacs::index_iterator(0UL), acmacs::index_iterator(titers->number_of_layers()),
                ...
                std::cout << std::setw(4) << std::right << antigen_no << ' ' << std::setw(max_antigen_name) << std::left << chart->antigen(antigen_no)->name_full()
