    ...
    if (mTiters.empty()) {
        ...
        if (num_less && num_more) {
            ...
            if (sd.population_sd() > sd_threshold_) {
                ...
                if (value != mMerged)
                    throw std::runtime_error(fmt::format("Unexpected merged titer: {} for sources: {}, expected: {}", *value, acmacs::to_string(mTiters), *mMerged));
            }
        }
    }
    return mType;
